...
-  `feat_ext_template.sh` — SLURM template for feature extraction
-  `feat_ext_navyblue.sh` — Batch submission for navyblue dataset
-  `feat_ext_pv3.sh` — Batch submission for pv3 dataset
-  `grid_search_train.sh` — Grid search for model training
-  `train_template.sh` — SLURM template for model training (not shown here)
-  `extract_features_fp.py` — Python script for feature extraction (not shown here)
-  `job_logs/` — Output and error logs from SLURM jobs

---

## Usage Notes

-  Update the `BASE_DIR` variable in the batch submission scripts to point to your data location.
-  Make sure to set the correct conda environment and module loads as needed for your cluster.
...
For questions, contact [Aiden Taghinia](mailto:aiden.taghinia@cuanschutz.edu).
